...
class AdaptiveBinarizer:
    ...
    def binarize(self, img: np.ndarray) -> np.ndarray:
        img = self.adjust_gamma(img)  #RGB -> BGR
        mask = self.get_mask(img) # три этапа
        image_in = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        image_out = self.combine_block_image_process(image_in, mask)
        image_out = cv2.cvtColor(image_out, cv2.COLOR_GRAY2RGB)
        return image_out

    def adjust_gamma(self, image: np.ndarray, gamma: float = 1.2) -> np.ndarray:
    ...
    def preprocess(self, image: np.ndarray) -> np.ndarray:
        image = cv2.medianBlur(image, 3)  # центральный пиксель изображения
                                          # заменяется медианой всех пикселей в области ядра - слегка размываем
        return 255 - image  # негатив
    ...
    def get_block_index(self, image_shape: Tuple[int, int], yx: Tuple[int, int]) -> np.ndarray:
    ...
    def block_image_process(self, image: np.ndarray) -> np.ndarray:
        out_image = np.zeros_like(image)
        for row in range(0, image.shape[0], self.block_size):
            for col in range(0, image.shape[1], self.block_size):
                idx = (row, col)
                block_idx = self.get_block_index(image.shape, idx)
    ...
    def get_mask(self, img: np.ndarray) -> np.ndarray:
        image_in = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        image_in = self.preprocess(image_in)
        image_out = self.block_image_process(image_in)
        image_out = self.postprocess(image_out)
        return image_out
    ...
    def combine_block_image_process(self, image: np.ndarray, mask: np.ndarray) -> np.ndarray:
        out_image = np.zeros_like(image)  # черный

        for row in range(0, image.shape[0], self.block_size):
            for col in range(0, image.shape[1], self.block_size):
                block_idx = self.get_block_index(image.shape, (row, col))
